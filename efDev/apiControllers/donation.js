...
function registrationAndDonate(req, res, next) {
    ...
    user.save(function(err, user) {
        if(err) {
            ...
            donation.project = new ObjectId(donation.projectId)
            donation.user = new ObjectId(userId);
            donation.confired = true;
            var donation = new Donation(donation);
...
function donate(req, res, next) {
   ...
   if(! (postData.projectId && postData.userId)) {
        ...
        res.send(400,'userId or projectId not found');
   } else {
